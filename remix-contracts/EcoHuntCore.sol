...
contract EcoHuntCore {
    ...
    function completeEcoAction(
        ...
        // Calculate reward
        uint256 totalReward = action.baseReward + ((profile.level * 5 * action.baseReward) / 100);
        ...
        if (profile.totalActions % 10 == 0) {
            _mintMilestoneNFT(user, actionType, action.carbonOffset, imageHash, profile.totalActions);
        }
    ...
    function _mintMilestoneNFT(
        ...
        uint256 tokenId = ecoNFT.mintEcoNFT(
            ...
            carbonOffset,
            imageHash,
            _generateNFTMetadata(user, actionType, actionCount)
        );
    ...
    function _generateNFTMetadata(
        ...
        return string(abi.encodePacked(
            "https://ecohunt-farcaster.vercel.app/nft/",
    ...
    function addEcoAction(
        ...
        _addEcoAction(actionType, baseReward, carbonOffset);
    }
    
    function updateEcoAction(
        string memory actionType,
        uint256 baseReward,
        uint256 carbonOffset,
        bool active
    ) external onlyOwner {
        ecoActions[actionType].baseReward = baseReward;
        ecoActions[actionType].carbonOffset = carbonOffset;
        ecoActions[actionType].active = active;
    }
